...
## 📝 최근 변경사항
- **refactor**: DB 접근을 repository 레이어로 분리 — `CustomerRepository`·`WebhookEventRepository` (v1.2.2)
- **fix(deps)**: itsdangerous 버전 핀 추가 (==2.2.0)
- **fix**: ruff E501·UP007 전면 수정, Python 3.11 venv 전환
- **fix**: 순환 임포트 해소, CI 환경 구성, 테스트 전면 수정
- **feat**: Migrate local dev to PostgreSQL and update dev setup
- **feat**: Add Python version and License badges to README.md
...
[CLAUDE.md](CLAUDE.md) (작업 규칙) · 변경 이력은 위 "최근 변경사항" 및 git 로그
